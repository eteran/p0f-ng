...
void tcp_register_sig(uint8_t to_srv, uint8_t generic, int32_t sig_class, uint32_t sig_name,
	...
	if (!strcmp((char *)val, "*"))
		pay_class = -1;
	else if (!strcmp((char *)val, "0"))
		pay_class = 0;
	else if (!strcmp((char *)val, "+"))
		pay_class = 1;
	else
	...
	tsig = (struct tcp_sig *)calloc(sizeof(struct tcp_sig), 1);
...
struct tcp_sig *fingerprint_tcp(uint8_t to_srv, struct packet_data *pk,
	...
	sig = (struct tcp_sig *)ck_alloc(sizeof(struct tcp_sig));
